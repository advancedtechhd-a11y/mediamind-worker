...
export async function searchArticles(query: string, maxResults: number = 10): Promise<TavilyResult[]> {
  if (!TAVILY_API_KEY) {
...
export async function searchNews(query: string, maxResults: number = 10): Promise<TavilyResult[]> {
  if (!TAVILY_API_KEY) {
    console.error('[Tavily] API key not configured');
    return [];
  }
...
export async function searchSite(site: string, query: string, maxResults: number = 5): Promise<TavilyResult[]> {
