...
async function getVideoInfo(videoPath) {
    try {
        const { stdout } = await execPromise(`ffprobe -v error -select_streams v:0 -show_entries stream=width,height -show_entries format=duration -of json "${videoPath}"`);
...
// EXTRACT CLIP
// ============================================
async function extractClip(videoPath, outputPath, startSeconds, duration) {
    try {
        console.log(`[FFmpeg] Extracting clip: ${startSeconds}s - ${startSeconds + duration}s`);
        await execPromise(`ffmpeg -i "${videoPath}" -ss ${startSeconds} -t ${duration} -c:v libx264 -c:a aac -preset fast "${outputPath}" -y`, { timeout: 120000 });
        return fs.existsSync(outputPath);
    }
    catch (error) {
...
// ============================================
async function extractFrames(videoPath, outputDir, intervalSeconds = 10) {
    try {
        if (!fs.existsSync(outputDir)) {
            fs.mkdirSync(outputDir, { recursive: true });
        }
        const fps = 1 / intervalSeconds;
        await execPromise(`ffmpeg -i "${videoPath}" -vf "fps=${fps}" "${outputDir}/frame_%04d.jpg" -y`, { timeout: 300000 });
...
//# sourceMappingURL=ffmpeg-standalone.js.map