...
function isRelevant(title, topic) {
    ...
    if (keywords.length === 0)
        return true; // If no keywords extracted, allow it
        ...
        try {
            ...
            for (const doc of docs.slice(0, 25)) {
                ...
                try {
                    ...
                    const videoFile = files.find((f) => f.name?.endsWith('.mp4') || f.name?.endsWith('.webm') || f.name?.endsWith('.ogv'));
                    if (videoFile) {
...
async function searchSearXNGVideos(topic, queries) {
...
async function searchFreeStockVideo(topic, queries) {
...
async function searchNewsDocumentary(topic, queries) {
...
async function searchStockFootage(topic, queries) {
...
async function searchWebVideos(topic, queries) {
...
//# sourceMappingURL=video-standalone.js.map