...
// DETECT AUDIO
// ============================================
export async function detectAudio(videoPath) {
    try {
        const { stdout } = await execPromise(`ffprobe -i "${videoPath}" -show_streams -select_streams a -loglevel error`);
...
export async function extractAudio(videoPath, audioPath) {
    try {
        await execPromise(`ffmpeg -i "${videoPath}" -vn -acodec libmp3lame -q:a 2 "${audioPath}" -y`);
...
// EXTRACT FRAMES
// ============================================
export async function extractFrames(videoPath, outputDir, intervalSeconds = 5) {
    try {
        ...
        await execPromise(`ffmpeg -i "${videoPath}" -vf "fps=${fps}" "${outputDir}/frame_%04d.jpg" -y`);
...
// EXTRACT CLIP
// ============================================
export async function extractClip(videoPath, outputPath, startSeconds, endSeconds) {
    try {
        const duration = endSeconds - startSeconds;
        await execPromise(`ffmpeg -i "${videoPath}" -ss ${startSeconds} -t ${duration} -c:v libx264 -c:a aac "${outputPath}" -y`);
        return fs.existsSync(outputPath);
    }
    catch (error) {
...
export async function getVideoInfo(videoPath) {
    try {
        const { stdout } = await execPromise(`ffprobe -v error -select_streams v:0 -show_entries stream=width,height,duration -show_entries format=duration -of json "${videoPath}"`);
